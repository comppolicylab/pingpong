...
| `azure.oai.engine` | The name of your Azure OpenAI GPT-4 deployment | _None_ | `stats-tutor-gpt4` |
| `azure.oai.temperature` | The temperature setting for the LLM | `0.2` | `0.2` |
| `azure.oai.top_p` | The `top_p` setting for the LLM | `0.95` | `0.95` |
| `azure.oai.api.base` | The URL to the OpenAI deployment | _None_ | `https://ai-tutor.openai.azure.com/` |
| `azure.oai.api.type` | The type of API endpoint we're using | `azure` | `azure` |
| `azure.oai.api.chat_version` | The API version to use | _None_ | `2023-06-01-preview` |
| `azure.oai.api.key` | The Azure OpenAI key | _None_ | `aaa111222333abcabc123abc987` |
| `azure.cs.endpoint` | The Cognitive Search endpoint URL | _None_ | `https://stats-tutor-cs.search.windows.net` |
| `azure.cs.key` | The Cognitive Search key | _None_ | `AAA111xxxxyyyAAAA111YYYdddDDDddDDDdD09` |
| `azure.cs.restrict_answers_to_data` | Whether to answer strictly in terms of documents in store. `false` will let the model draw on its training to function more like a general-purpose ChatGPT. | `true` | `false` |
| `azure.cs.type` | Type of Cognitive Search deployment (probably no need to set this explicitly) | `AzureCognitiveSearch` | `AzureCognitiveSearch` |
...
| `tutor.channels[].cs_index_name` | Cognitive search index name to use for semantic search | _None_ | `cs-aitutor-idx` |
