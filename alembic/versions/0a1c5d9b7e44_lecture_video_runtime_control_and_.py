...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("state", lecture_video_session_state_enum, server_default="PLAYING", nullable=False),
        sa.Column("current_question_id", sa.Integer(), nullable=True),
        sa.Column("active_option_id", sa.Integer(), nullable=True),
        sa.Column("last_known_offset_ms", sa.Integer(), server_default="0", nullable=False),
        sa.Column("version", sa.Integer(), server_default="1", nullable=False),
        sa.Column("controller_session_id", sa.String(), nullable=True),
        sa.Column("controller_user_id", sa.Integer(), nullable=True),
        sa.Column("controller_lease_expires_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column(
            "created_at",
        ...
        sa.ForeignKeyConstraint(["active_option_id"], ["lecture_video_question_options.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["controller_user_id"], ["users.id"]),
        sa.ForeignKeyConstraint(["current_question_id"], ["lecture_video_questions.id"], ondelete="SET NULL"),
    ...
    op.create_table(
        ...
        sa.Column("event_type", lecture_video_interaction_event_type_enum, nullable=False),
        ...
        sa.ForeignKeyConstraint(["option_id"], ["lecture_video_question_options.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["question_id"], ["lecture_video_questions.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["thread_id"], ["threads.id"], ondelete="CASCADE"),
