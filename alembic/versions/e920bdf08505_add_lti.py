...
def downgrade() -> None:
    ...
    sa.Enum('PENDING', 'APPROVED', 'REJECTED', name='ltiregistrationreviewstatus').drop(op.get_bind(), checkfirst=True)
    sa.Enum('RS256', name='tokenalgorithm').drop(op.get_bind(), checkfirst=True)
    # ### end Alembic commands ###
