...
async def merge_external_logins(
    ...
    logger.info(
        f"(merge_external_logins) ExternalLogin RID before migration: {await ExternalLogin.get_last_row_id(session)}"
    )

    for provider, identifier in old_logins:
        logger.info(
            f"(merge_external_logins) ExternalLogin RID before {provider}, {identifier} migration: {await ExternalLogin.get_last_row_id(session)}"
        )
        ...
        logger.info(
            f"(merge_external_logins) ExternalLogin RID after {provider}, {identifier} migration: {await ExternalLogin.get_last_row_id(session)}"
        )

    logger.info(
        f"(merge_external_logins) ExternalLogin RID after migration: {await ExternalLogin.get_last_row_id(session)}"
    )
