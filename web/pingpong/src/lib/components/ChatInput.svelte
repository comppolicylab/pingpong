...
<div use:init={$page.params.threadId} class="w-full relative">
  <Textarea id="message" rows="1" name="message" placeholder="Ask me anything" disabled={loading || disabled} on:keydown={maybeSubmit} on:input={e => fixHeight(e.target)}
    ...
    <GradientButton type="submit" color="cyanToBlue" class={`${loading ? "animate-pulse cursor-progress" : ""} p-2 absolute bottom-3 right-2.5`} disabled={loading || disabled}><ChevronUpSolid size="xs" /></GradientButton>
</div>
