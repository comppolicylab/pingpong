    ...
    <div class="flex flex-col gap-4">
      ...
      <div>
        ...
        {#if activeTool.input_schema}
          <pre
            class="text-xs whitespace-pre-wrap font-mono text-gray-700 rounded border border-gray-200 bg-gray-50 px-4 py-3">{formatPayload(
...
<div class={compact ? 'my-2' : 'my-3'}>
  {#if showServerLabel}
    <div class="flex items-center gap-2 text-gray-600">
      ...
      <span class="text-xs font-medium uppercase tracking-wide">{serverLabel}</span>
    </div>
  {/if}
