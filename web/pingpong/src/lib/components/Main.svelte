...
<div
	class={`main-panel absolute top-24 left-0 z-10 mr-4 ml-4 h-[calc(100%-6rem)] w-[calc(100%-2rem)] overflow-hidden transition-all print:!static print:!top-0 print:!left-0 print:!z-auto print:!m-0 print:!h-auto print:!w-full print:!overflow-visible print:!p-0  ${
		$appMenuOpen ? (forceCollapsedLayout && forceShowSidebarButton ? 'left-80 ml-0! lg:ml-4!' : 'left-[90%] md:left-80') : ''
	} ${!inIframe || !forceCollapsedLayout ? 'lg:static lg:h-full' : ''}`}
>
	<div
		class="relative h-full grow overflow-hidden {isLtiHeaderLayout ? 'rounded-tl-4xl' : 'rounded-t-4xl'} {$appMenuOpen && forceCollapsedLayout && forceShowSidebarButton ? 'md:rounded-tl-none' : ''}  bg-white print:!h-auto print:!overflow-visible print:!rounded-none print:!bg-transparent transition-all"
	>
