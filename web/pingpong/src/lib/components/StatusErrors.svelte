...
{#if assistantStatusUpdates.length > 0}
	<div class="mb-2 w-full text-xs text-gray-700 xl:text-sm">
		<div class="mx-4 flex flex-col gap-1.5">
			{#if hasMultipleIssues}
				<button
					type="button"
					class="flex w-full items-center gap-3 text-left text-[11px] font-semibold tracking-[0.18em] text-gray-600 uppercase transition hover:text-gray-800 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-sky-400/60 focus-visible:ring-offset-2"
			...
			{#if showDetails}
				<div class="flex flex-col gap-1.5" transition:slide={{ duration: 220 }}>
					{#each assistantStatusUpdates as statusUpdate (statusUpdate.incidentId)}
						...
						>
							<span
								class={`-mr-1 inline-block h-2 w-2 shrink-0 rounded-full ${impactStyle.dot}`}
								aria-hidden="true"
							></span>
							<div class="flex min-w-0 items-baseline gap-2">
								<span class="text-xs font-semibold tracking-wide whitespace-nowrap uppercase">
