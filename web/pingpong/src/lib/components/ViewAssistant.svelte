<script lang="ts">
  ...
  import { EyeOutline, EyeSlashOutline, LinkOutline, PenSolid, CirclePlusSolid } from 'flowbite-svelte-icons';
...
<div class="flex flex-col gap-2 {editable? 'bg-gold-light' : 'bg-orange-light'} rounded-2xl px-8 pt-6 py-4 pr-4 pb-8">
  <Heading tag="h4" class="flex gap-4 text-3xl font-normal items-center">
    ...
    <div class="ml-auto flex shrink-0 items-center gap-2">
      {#if editable}
        <a class="text-blue-dark-30 hover:text-blue-dark-50" href="/class/{assistant.class_id}/assistant/{assistant.id}"><PenSolid size="md" /></a>
      {/if}
  ...
  <div>
    <a href={assistantLink} class="flex items-center w-36 gap-2 text-sm text-white font-medium bg-orange rounded-full p-2 px-4 hover:text-blue-dark-100 hover:bg-blue-dark-40 hover:text-white transition-all">Start a chat <CirclePlusSolid size="sm" class="inline" /></a>
  </div>
</div>
