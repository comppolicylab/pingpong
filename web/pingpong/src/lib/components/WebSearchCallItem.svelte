...
<div class="my-3">
	{#if content.action && content.action.type === 'search' && uniqueSources.length > 0}
		<div class="flex items-center gap-2">
			...
			<button class="items-bottom flex flex-row" onclick={handleClick}>
				{#if content.status === 'completed'}
					<span class="text-left text-sm font-medium text-gray-600"
				...
				{:else}
					<span class="shimmer text-sm font-medium"
		...
		{#if open}
			<div
				class="ml-2 flex w-3/4 flex-row flex-wrap gap-x-2 gap-y-1 border-l border-gray-200 py-2 pl-4 text-sm font-light text-gray-600"
			...
			<div class="items-bottom flex flex-row">
				{#if content.action.type === 'search'}
					...
					{:else if content.status === 'incomplete'}
						<span class="text-sm font-medium text-yellow-600">Web search was canceled</span>
					{:else}
						<span class="shimmer text-sm font-medium">Searching web...</span>
					{/if}
				{:else if content.action.type === 'find'}
					...
					{:else if content.status === 'incomplete'}
						<span class="text-sm font-medium text-yellow-600">Web search was canceled</span>
					{:else}
						<span class="shimmer text-sm font-medium">Digging through web sources...</span>
					{/if}
				{:else if content.action.type === 'open_page'}
					...
					{:else if content.status === 'incomplete'}
						<span class="text-sm font-medium text-yellow-600">Web search was canceled</span>
					{:else}
						<span class="shimmer text-sm font-medium">Looking through web sources...</span>
					{/if}
				{/if}
			</div>
		</div>
