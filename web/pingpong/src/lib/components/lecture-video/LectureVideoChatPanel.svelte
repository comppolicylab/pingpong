<script lang="ts">
	...
	const scroll = (el: HTMLDivElement, params: ScrollParams) => {
		...
		const onScroll = () => {
			...
			if (isScrollingUp) userPausedAutoScroll = true;
			if (isStreaming) {
	...
	<div class="border-t border-slate-200 px-4 py-3">
		<div class="relative mx-auto flex w-full max-w-4xl flex-col">
			...
			<ChatInput
				...
				on:dismissError={() => dispatch('dismissError')}
				on:textinput={(e) => dispatch('textinput', e.detail)}
				on:textpaste={(e) => dispatch('textpaste', e.detail)}
			/>
		</div>
	</div>
</div>
