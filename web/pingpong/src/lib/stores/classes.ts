...
export class ClassesManager {
    ...
    async load(force: boolean = false, pageSize: number = this.#defaultPageSize) {
        ...
        if (response.error) {
            this.#data.update($data => ({ ...$data, loading: false, error: response.error, classes: [], lastPage: true }));
            return;
        } else {
            this.#data.update($data => ({ ...$data, loading: false, classes: response.data.classes, lastPage: true }));
        }
    }
