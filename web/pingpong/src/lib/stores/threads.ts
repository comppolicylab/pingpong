...
class ThreadsManager {
     ...
     */
    async load(classId: number, force: boolean = false, pageSize: number = this.#defaultPageSize, background: boolean = false) {
        ...
        this.#data.update(d => ({...d, error: null, threads: background ? d.threads : [], loading: !background, classId}));
        ...
        if (response.error) {
            this.#data.update(d => {
                ...
                return {...d, loading: false, error: response.error, threads: [], lastPage: response.lastPage};
            });
            return;
        } else {
            this.#data.update(d => {
                ...
                return {...d, loading: false, error: null, threads: sortThreads(response.threads), lastPage: response.lastPage};
            });
        }
    }
    ...
    async loadMore(pageSize: number = this.#defaultPageSize) {
        ...
        const response = await api.getClassThreads(this.#fetcher, current.classId, {limit: pageSize, before: lastThread?.created});
        if (response.error) {
            this.#data.update(d => {
                // If the class ID has changed, drop the update
            ...
            this.#data.update(d => {
                ...
                return {...d, loading: false, error: null, threads: sortThreads([...d.threads, ...response.threads]), lastPage: response.lastPage};
            });
        }
    }
