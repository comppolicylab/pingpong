<script lang="ts">
  ...
  import { Button, Checkbox, Heading, Helper, Hr, Input, Label, Modal, Table, TableBody, TableBodyCell, TableBodyRow, TableHead, TableHeadCell, Textarea, Toggle, Tooltip } from 'flowbite-svelte';
  import { ArrowRightOutline, CheckOutline, PlusOutline, QuestionCircleOutline } from 'flowbite-svelte-icons';
  ...
  const handleActiveStatusChange = async (event: Event, agreementId: number) => {
    ...
    if (agreementResponse.error) {
      return sadToast(agreementResponse.error.detail || `Unknown error ${activate ? 'activating' : 'deactivating'} agreement`);
    }
  ...
  <div class="h-full w-full overflow-y-auto p-12">
    <div class="flex flex-row flex-wrap justify-between mb-4 items-center gap-y-4">
      ...
      <Button
              ...
              href="/admin/terms/create"><PlusOutline />Create an Agreement</Button
            >
    </div>
    <div class="flex flex-col gap-4">
      {#each data.categories as category}
      {@const userAgreements_ = userAgreements.filter((agreement) => agreement.category.id === category.id)}
          ...
          <Table class="w-full overflow-visible">
            ...
            <TableBody>
              {#each userAgreements_ as agreement}
                <TableBodyRow>
                  <TableBodyCell class="py-2 font-medium whitespace-normal">{agreement.name}</TableBodyCell>
                  ...
                  <TableBodyCell class="flex flex-row gap-2 py-2 font-normal whitespace-normal">
                    {#if agreement.always_display || agreement.category.show_all}<CheckOutline />{#if !agreement.always_display && agreement.category.show_all}
                    ...
                    >
                      <div class="normal-case whitespace-normal">
                        <p>Enforced by {agreement.category.name}'s <i>Show All Agreements</i> property</p>
                      </div>
                    </Tooltip>{/if}{/if}
                  </TableBodyCell>
                  <TableBodyCell class="py-2 font-normal whitespace-normal">
                  ...
                  <TableBodyCell class="py-2">
                    <Toggle
                      ...
                      on:change={(event) =>
                        handleActiveStatusChange(event, agreement.id)}
                    />
                  </TableBodyCell>
                  <TableBodyCell class="py-2">
                    <Button
      ...
      <div>
            <Table class="w-full">
              ...
              <TableBody>
                {#each data.categories as category}
                  <TableBodyRow>
                    <TableBodyCell class="py-2 font-medium whitespace-normal">{category.name}</TableBodyCell>
                    <TableBodyCell class="py-2 font-normal whitespace-normal">
...
<Modal bind:open={editModalOpen} size="sm" on:close={() => (categoryToEdit = null)} >
  <form class="flex flex-col space-y-6 pb-4" action="#" on:submit={submitCategoryChange}>
    ...
    <Label class="space-y-2">
      ...
      <Helper class="mb-2">By default, only the latest agreement will be shown for users to read and agree to. When enabled, all agreements with this agreement type will be shown to users sequentially.</Helper>
      <Checkbox
        id="show_all"
        name="show_all"
        checked={categoryToEdit?.show_all}
